...
vec3 spotlight(vec2 uv, vec2 pos, float angle, float fog) {
    vec2 muv = uv;
    muv -= pos;
    ...
    uv2.x = smoothstep(0., 1., uv2.x);
    float osc = sin(angle * 1e-1) * 0.5 + 0.5;
    float x = fract(angle * 1e-2) * 2.;
    osc = fract(x)*5.*(-1.+floor(x)*2.)+(1.-floor(x));
    osc = clamp(osc, 0., 1.);
    float oscNoFloor = osc;
    osc = max(osc, max(0.,(fract(x)*-0.3+0.3)*(1.-floor(x))));
    float hue = mod(floor(((fract(angle * 0.5e-2) * 4.)+1.)*0.5), 2.);
    float circle;
    if (osc < 0.4) {
...
void main() {
    ...
    for (float i = 0.0; i < pi * 2.; i+= pi * 2. / 15.) {
        ...
        vec3 c = spotlight(uv, vec2(x * 3., y * 3.), -time*2.+i*3e1, fog);
