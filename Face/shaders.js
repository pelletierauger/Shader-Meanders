...
// Blue shappire with polar voronoise and radial oscillation
setBothShaders(`
// beginGLSL
precision mediump float;
#define pi 3.1415926535897932384626433832795
...
void main() {
    ...
    // tri = smoothstep(0.95, 1., tri) + smoothstep(0.5, 1., tri) * 0.5;
    float interior = (smoothstep(0.95, 1., tri));
    tri = smoothstep(0.7, 1., tri) * 1.0 - 
        (smoothstep(0.99, 1., tri)) + 
        (1.0 - smoothstep(1., 1.05, tri) * 2.1) * 
        smoothstep(0.99, 1., tri) * 0.5;
    vec2 polar = vec2((atan(uv.y,uv.x)), length(uv));
    float voro = voronoise(polar*10., 0.75, 0.45);
    // float voro2 = voronoise(uv*12., 0.75, 0.3);
    voro *= pow((polar.x/pi), 2.) * -1. + 1.;
    ...
    float voro3 = voronoise(polar*14.+vec2(1.,1.), 0.75, 0.3);
    // voro = mix(voro, voro3, 0.5);
    float hl = map(sin(elli*10.+time*0.75e-1+22.),-1.,1.,0.85,1.) * 0.9;
    voro *= smoothstep(0., 1., pow(hl,2.));
    voro += smoothstep(0., 1., pow(hl,24.))*10.;
    voro3 *= smoothstep(0., 1., pow(hl,2.));
    voro3 += smoothstep(0., 1., pow(hl,24.))*10.;
    voro3 *= pow((polar.x/pi), 2.) * -1. + 1.;
    // voro3 *= 1.0-smoothstep(1.2, 1.5, elli);
    voro3 *= min(1., length(uv)*6.);
    // voro = voro3;
    voro = mix(voro, voro3, 0.5);
    // voro = mix(voro, voro3, map(sin(-time*1e-1),-1.,1.,0.,1.));
    tri = max(tri, (interior * voro));
    // tri *= map(sin((uv.x+uv.y)*5.+time*0.75e-1),-1.,1.,0.25,1.);
    tri *= map(sin(length(uv)*10.-time*0.75e-1),-1.,1.,0.25,1.);
    // tri = abs(tri - 0.5) * -1. + 0.5;
    tri = smoothstep(0., 1., tri);
    float noise = rand(uv + sin(time)) * 0.075;
    vec3 col = vec3(1.0, 0.0, 0.0);
    ...
    col = col * tri;
    col = hueShift(col, 3.8);
    // col = mix(col, Desaturate(col, 1.0-tri).rgb * 1.2, 0.5);
    gl_FragColor = vec4(col, 1. - noise);
}
// endGLSL
`);
